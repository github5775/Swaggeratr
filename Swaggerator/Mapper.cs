...
namespace Swaggerator
{
    internal class Mapper
    {
        ...
        private bool IsValidInterfaceCollection(Type[] interfaces)
        {
            ...
            if (interfaces.Length == 1 && interfaces[0].Name == "IDisposable")
            {
                return false;
            }
        ...
        internal IEnumerable<Tuple<string, Operation>> GetOperations(MethodInfo[] methodInfos, Stack<Type> typeStack)
        {
            ...
            for (int index = 0; index < methodInfos.Count(); index++)
            {
                ...
                //if the method is marked Hidden anywhere, skip it
                if (implementation.GetCustomAttribute<HiddenAttribute>() != null) { continue; }

                //if a tag from either implementation or declaration is marked as not visible, skip it
                var methodTags = implementation.GetCustomAttributes<TagAttribute>().Select(t => t.TagName);
                if (methodTags.Any(HiddenTags.Contains)) { continue; }

                //find the WebGet/Invoke attributes, or skip if neither is present
                WebGetAttribute wg = implementation.GetCustomAttribute<WebGetAttribute>();
                WebInvokeAttribute wi = implementation.GetCustomAttribute<WebInvokeAttribute>();
                if (wg == null && wi == null) { continue; }
                ...
                var returnType =
                    Helpers.GetCustomAttributeValue<Type, OverrideReturnTypeAttribute>(implementation, "Type") ?? implementation.ReturnType;

                var returnTypeString = HttpUtility.HtmlEncode(Helpers.MapSwaggerType(returnType, typeStack));
                returnTypeString = Helpers.GetDataContractNamePropertyValue(returnType) ?? returnTypeString;
                List<string> acceptsParams = new List<string>();
                List<string> producesParams = new List<string>();

                if (wg != null)
                {
                    if (wg.IsRequestFormatSetExplicitly)
                    {
                        acceptsParams.Add("application/" + wg.RequestFormat.ToString().ToLower());
                    }
                    else
                    {
                        acceptsParams = new List<string>(GetContentTypes<AcceptsAttribute>(implementation, null));
                    }
                    if (wg.IsResponseFormatSetExplicitly)
                    {
                        producesParams.Add("application/" + wg.ResponseFormat.ToString().ToLower());
                    }
                    else
                    {
                        producesParams = new List<string>(GetContentTypes<ProducesAttribute>(implementation, null));
                    }
                }
                else  //wi not null
                {
                    if (wi.IsRequestFormatSetExplicitly)
                    {
                        acceptsParams.Add("application/" + wi.RequestFormat.ToString().ToLower());
                    }
                    else
                    {
                        acceptsParams = new List<string>(GetContentTypes<AcceptsAttribute>(implementation, null));
                    }
                    if (wi.IsResponseFormatSetExplicitly)
                    {
                        producesParams.Add("application/" + wi.ResponseFormat.ToString().ToLower());
                    }
                    else
                    {
                        producesParams = new List<string>(GetContentTypes<ProducesAttribute>(implementation, null));
                ...
                Operation operation = new Operation
                {
                    ...
                    summary = summary,
                    notes = description,
                    accepts = acceptsParams,
                    produces = producesParams
                };

                if (implementation.ReturnType.IsArray)
                {
                    operation.items = new OperationItems
                    {
                        Ref = Helpers.MapElementType(implementation.ReturnType, typeStack)
                    };
                }

                operation.errorResponses.AddRange(GetResponseCodes(methodInfos[index]));
                ...
                //try to map each implementation parameter to the uriTemplate.
                ParameterInfo[] parameters = implementation.GetParameters();
                foreach (ParameterInfo parameter in parameters)
                {
                    var typeValue = HttpUtility.HtmlEncode(Helpers.MapSwaggerType(parameter.ParameterType, typeStack));
                    typeValue = Helpers.GetDataContractNamePropertyValue(parameter.ParameterType) ?? typeValue;
        ...
        private IEnumerable<string> GetContentTypes<T>(MethodInfo implementation, MethodInfo declaration) where T : ContentTypeAttribute
        {
            ...
            else if (declaration != null && declaration.GetCustomAttributes<T>().Any())
            {
