...
namespace Swaggerator
{
    internal class Serializer
    {
        ...
        private int CountSlashesInUrl(string url)
        {
            return CountCharInString('/', url);
        }
        private int CountCharInString(char ch, string ss)
        {
            int count = 0;
        ...
        internal string WriteApi(Uri basePath, string servicePath, Type serviceType, Stack<Type> typeStack)
        {
            ...
            using (JsonWriter writer = new JsonTextWriter(sw))
            {
                ...
                string path = string.Format("{0}://{1}", basePath.Scheme, basePath.Authority);
                string absoluteUri = basePath.AbsoluteUri;
                int position = absoluteUri.IndexOf(servicePath);
                if (position > -1)
                {
                    path = absoluteUri.Substring(0, position - 1);

                    int count = CountSlashesInUrl(path);
                    if (count > 1)
                    {
